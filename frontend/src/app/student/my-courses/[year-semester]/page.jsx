...
export default function Page() {
  ...
  // Before publication, show every enrolled course, including failed courses.
  // After publication, failed courses are supplied by the semester result so
  // they can be merged into the same table without duplicating rows.
  ...
  const isSemesterPublished = Boolean(semesterResult?.published);
  ...
  const tableCourses = useMemo(() => {
    if (!semesterResult?.published) return courses;

    const failedCourses = (semesterResult.failed_courses || []).map((course, index) => ({
      ...course,
      id: `failed-${course.course_code || index}`,
      status: "failed",
    }));

    return [...currentCourses, ...failedCourses];
  }, [courses, currentCourses, semesterResult]);
  const tableCount = isSemesterPublished ? tableCourses.length : count;
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {!isValidSlug ? (
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
            <DataTableToolbar search={search} setSearch={updateSearch} ordering={ordering} setOrdering={updateOrdering} searchPlaceholder="Search courses..." count={tableCount} countLabel="Courses" orderingOptions={[{ value: "-enrolled_at", label: "Newest Enrolled" }, { value: "enrolled_at", label: "Oldest Enrolled" }]} />
            {loadingYearSemesters || isLoading || isFetching ? <div className="p-10 text-center text-muted-foreground"><Loader2 className="mx-auto h-6 w-6 animate-spin" /><p className="mt-2 text-sm">Loading courses...</p></div> : tableCourses.length === 0 ? <div className="p-10 text-center"><BookOpen className="mx-auto h-10 w-10 text-muted-foreground" /><h2 className="mt-3 font-medium text-foreground">No Courses Found</h2><p className="mt-2 text-sm text-muted-foreground">You have no enrolled courses for this semester.</p></div> : (
              <div className="overflow-x-auto"><table className="w-full min-w-max"><thead className="bg-muted/50"><tr><th className="px-6 py-4 text-left text-sm font-semibold text-muted-foreground">Course</th><th className="px-6 py-4 text-center text-sm font-semibold text-muted-foreground">Status</th><th className="px-6 py-4 text-center text-sm font-semibold text-muted-foreground">Grade Letter</th><th className="px-6 py-4 text-center text-sm font-semibold text-muted-foreground">Grade Point</th><th className="px-6 py-4 text-center text-sm font-semibold text-muted-foreground">Contents</th></tr></thead><tbody>{tableCourses.map((course) => { const grade = course.letter_grade ? course : gradeByCode[course.course_code]; return (<tr key={course.id} className="border-t border-border transition hover:bg-accent/50"><td className="px-6 py-4"><p className="font-medium text-foreground">{course.course_title || "-"}</p><p className="text-sm text-muted-foreground">{course.course_code || "-"}</p></td><td className="px-6 py-4 text-center"><span className={`inline-flex rounded-md px-2 py-0.5 text-sm font-medium ${statusStyles[course.status] || "bg-muted text-muted-foreground"}`}>{course.status?.toUpperCase() || "-"}</span></td><td className="px-6 py-4 text-center font-medium text-foreground">{isSemesterPublished ? grade?.letter_grade || "-" : "-"}</td><td className="px-6 py-4 text-center font-medium text-foreground">{isSemesterPublished && grade?.grade_point != null ? Number(grade.grade_point).toFixed(2) : "-"}</td><td className="px-6 py-4"><div className="flex justify-center"><ManageMenu sessionCourseId={course.session_course} base={base} /></div></td></tr>); })}</tbody></table></div>
            )}

...
class AttendanceSessionView(GenericAPIView):
    ...
    @extend_schema(
        summary="Attendance Session List",
        responses={200: AttendanceSessionSerializer(many=True)},
    )
    def get(self, request, session_course_id):

        session_course = SessionCourse.objects.filter(
        ...
        return Response(
            {
                "detail": "Attendance sessions retrieved successfully.",
        ...
        return Response(
            {
                ...
                "data": AttendanceSessionSerializer(
                    attendance_session,
                ).data,
            },
